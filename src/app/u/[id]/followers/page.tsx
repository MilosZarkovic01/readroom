...
export default async function FollowersPage({
  ...
  return (
    <AppShell>
      <h1 className="mb-1 font-sans text-[32px] font-medium text-espresso">Followers</h1>
      <p className="mb-5 font-sans text-sm text-warm-gray">{displayName(user)}</p>
      <FollowList
        empty="No followers yet."
        users={rows.map((row) => ({
          ...row.follower,
          following: followingIds.has(row.follower.id),
          isSelf: row.follower.id === session.user.id,
        }))}
      />
    </AppShell>
  );
